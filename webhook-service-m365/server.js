...
/** postMessage target: app origin from redirect URI (fallback '*' if parse fails) */
function getGoogleOAuthPostMessageTarget() {
    try {
...
app.get('/auth/google/callback', async (req, res) => {
    ...
    const postTarget = JSON.stringify(getGoogleOAuthPostMessageTarget());
    const closeScript = (success, errMsg) => {
        ...
        return `<script>window.opener && window.opener.postMessage(${payload}, ${postTarget}); window.close();</script>`;
    };
